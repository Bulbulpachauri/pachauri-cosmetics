...
import { Truck, Leaf, BadgeCheck, Sparkles, Star, ArrowRight } from "lucide-react";
    ...
    <>
        ...
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-14 lg:grid-cols-2 lg:items-center lg:px-8 lg:py-20">
          <div className="relative z-10">
            ...
            <h1 className="mt-4 font-display text-5xl leading-[1.05] text-ink sm:text-6xl">
              Beauty that feels
              <br />
              <span className="italic text-rose">like second skin</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/shop" className="btn-primary">Shop Now <ArrowRight size={15} /></Link>
              <Link to="/shop" className="btn-outline">Browse Categories</Link>
            </div>
          </div>
        ...
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-6 px-5 py-6 lg:grid-cols-4 lg:px-8">
          {TRUST.map(({ icon: Icon, label }) => (
            <div key={label} className="flex items-center gap-2.5">
              <Icon size={17} className="shrink-0 text-rose" strokeWidth={1.5} />
              <span className="font-body text-xs text-ink/70 sm:text-sm">{label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 text-center lg:px-8 lg:py-24">
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {CATEGORIES.map((c) => (
            <Link key={c.id} to={`/shop?cat=${c.id}`} className="group text-left">
      ...
      <section className="mx-auto max-w-6xl px-5 pb-16 lg:px-8 lg:pb-24">
        <div className="flex items-end justify-between">
          <div>
            ...
            <h2 className="section-title mt-2 !text-3xl">Featured Products</h2>
          </div>
          <Link to="/shop" className="font-body text-xs uppercase tracking-wide text-rose hover:underline">
            View all products
          </Link>
        </div>
      ...
      <section className="bg-ink text-white">
        <div className="mx-auto grid max-w-6xl gap-8 px-5 py-16 lg:grid-cols-2 lg:items-center lg:px-8">
          <div>
            <p className="font-body text-[11px] uppercase tracking-widest2 text-rose-light">Special Offer</p>
            <h2 className="mt-3 font-display text-4xl">Up to 30% off the Glow Edit</h2>
            <p className="mt-4 max-w-sm text-sm text-white/60">
              Bundle any serum, moisturiser and cleanser and the lowest-priced
              item is on us. Ends Sunday, while stocks last.
            </p>
            <Link to="/shop?cat=skincare" className="btn-dark mt-7">Shop the Offer <ArrowRight size={15} /></Link>
          </div>
          <div className="grid grid-cols-3 gap-4">
            {["p3", "p6", "p7"].map((id) => {
              const p = PRODUCTS.find((x) => x.id === id);
              return (
                <img key={id} src={p.image} alt={p.name} className="aspect-[3/4] w-full object-cover" />
              );
            })}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-5 py-16 lg:px-8 lg:py-24">
        <div className="flex items-end justify-between">
          <div>
            ...
            <h2 className="section-title mt-2 !text-3xl">Best Sellers</h2>
          </div>
          <Link to="/shop" className="font-body text-xs uppercase tracking-wide text-rose hover:underline">
            See top rated
          </Link>
        ...
        <div className="mx-auto max-w-6xl px-5 py-16 lg:px-8 lg:py-24">
          <div className="flex items-end justify-between">
            <div>
              ...
              <h2 className="section-title mt-2 !text-3xl">New Arrivals</h2>
            </div>
            <Link to="/shop" className="font-body text-xs uppercase tracking-wide text-rose hover:underline">
              Shop new in →
            </Link>
      ...
      <section className="bg-blush">
        <div className="mx-auto max-w-6xl px-5 py-16 text-center lg:px-8 lg:py-24">
          ...
          <h2 className="section-title mt-3">{STATS.rating} from {STATS.count} reviews</h2>
              ...
              <div key={r.name} className="card p-6 text-left">
                <div className="flex text-gold">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={13} fill={i < r.rating ? "#B68A56" : "none"} strokeWidth={1.5} />
                  ))}
                </div>
                <p className="mt-3 text-sm italic text-ink/70">"{r.text}"</p>
                <p className="mt-4 font-body text-sm font-medium text-ink">{r.name}</p>
      ...
      <section className="bg-blushdeep">
        <div className="mx-auto max-w-xl px-5 py-16 text-center lg:py-20">
          <p className="eyebrow justify-center">The Pachauri Cosmetics Letter</p>
          <h2 className="section-title mt-3 !text-3xl">15% off your first ritual</h2>
          <p className="mt-3 text-sm text-ink/60">
            Join for early access to launches, expert routines and members-only offers.
          </p>
          <form className="mx-auto mt-6 flex max-w-sm gap-2" onSubmit={(e) => e.preventDefault()}>
            <input className="input flex-1" placeholder="you@email.com" type="email" required />
